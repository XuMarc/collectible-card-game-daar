...
function CollectionsPage() {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:3001/getCollections')
      .then(response => {
        setCollections(response.data.data)
        console.log('COLLECTSIONS : ', collections[0])
      })
      .catch(error => {
  ...
  const createCollection = async (
    ...
    try {
      ...
      const response = await axios.post(
        'http://localhost:3001/createCollection',
        {
          id: collectionId,
          name: collectionName,
          img: collectionImg,
  ...
  return (
    <div className="p-6 bg-gray-900 min-h-screen text-white">
      <h1 className="text-4xl font-bold mb-6 text-center">
        Ensembles de Cartes Pokémon
      </h1>
      <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {collections.map(collection => (
          <li
            key={collection.id}
            className="collection-item bg-gray-800 rounded-lg p-4 shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col"
          >
            <Link
            ...
            <button
              ...
              className={`mt-2 w-full py-2 rounded-md font-bold transition duration-300 ${
                creatingCollection === collection.id
                  ? 'bg-gray-600'
